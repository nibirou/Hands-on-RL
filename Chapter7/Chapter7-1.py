...
if __name__ == "__main__":
    ...
    for i in range(10):
        with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / 10)):
                episode_return = 0
                # 3. 新版 reset 返回 (state, info)，并在此处设置 seed
                ...
                while not done:
                    ...
                    # 5. 将 terminated 和 truncated 合并为 done
    ...
    # 注意：请确保当前目录下存在 Chapter7 文件夹，否则会报路径找不到的错
    plt.savefig('./Chapter7/7_DQN_on_CartPole.png')  
    # plt.show()
    ...
    plt.ylabel('Returns')
    plt.title('DQN on {}'.format(env_name))
    plt.savefig('./Chapter7/7_DQN_moving_average_on_CartPole.png')
    # plt.show()